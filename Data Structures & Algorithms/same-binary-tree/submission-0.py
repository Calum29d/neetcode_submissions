...
class Solution:
    def isSameTree(self, p: Optional[TreeNode], q: Optional[TreeNode]) -> bool:
        ...
        #if one of the trees hits the bottom at a different time then we know that they arent equal
        if p is None or q is None:
            return False

        #and obviously if the values of the nodes arent equal then the trees arent equal
        if p.val != q.val:
            return False
